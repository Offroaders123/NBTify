...
*/
export async function write<T extends RootTag = never>(data: T | NBTData<T>, options?: WriteOptions): Promise<Uint8Array>;
export async function write<T extends RootTag = never>(data: T | NBTData<T>, { name, endian, compression, bedrockLevel }: WriteOptions = {}): Promise<Uint8Array> {
...
export class NBTWriter {
  ...
  */
  write<T extends RootTag = never>(data: T | NBTData<T>, options?: NBTWriterOptions): Uint8Array;
  write<T extends RootTag = never>(data: T | NBTData<T>, { name, endian }: NBTWriterOptions = {}): Uint8Array {
    if (data instanceof NBTData){
      if (name === undefined) name = data.name;
      if (endian === undefined) endian = data.endian;
      data = data.data;
